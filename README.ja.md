...
9. デプロイが完了するのを待ち、生成されたドメイン名をクリックして Dreame API にアクセスします。
...
アクセストークンを使って Dreame API にアクセスすることができる。使い方は [OpenAI API](https://platform.openai.com/docs/api-reference/introduction) と同じです。

OpenAI API が使用されている場所では、API Base に Dreame API のデプロイアドレスを設定することを忘れないでください（例: `https://openai.justsong.cn`）。API Key は Dreame API で生成されたトークンでなければなりません。
...
## FAQ
1. ノルマとは何か？どのように計算されますか？Dreame API にはノルマ計算の問題はありますか？
